...
url = "https://raw.githubusercontent.com/datadista/datasets/master/COVID%2019/ccaa_covid19_uci.csv"
uci = read.csv(url)
...
uci = data.frame(t(uci))

# Asigno el nombre de las columnas como las CCAA
colnames(uci) = c(comunidades)

# Manipulación de fechas
fechas = row.names(uci)

source("my_functions.R")
fechas = to_date(fechas)

uci$Fechas = fechas
row.names(uci) = seq(1, nrow(uci), 1)
